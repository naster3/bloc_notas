...
class Ventana(Frame):
    ...
    def widgets(self):
        ...
        archivo.add_command(label="Ventana Nueva",
                            command=self.segunda_ventana)
        ...
        edicion.add_command(
            label="Deshacer", command=lambda: self.texto.edit_undo())
        ...
        formato.add_checkbutton(label="Ajustes de linea",
                                variable=self.señal_ajustes, command=self.ajustes_de_linea)
        formato.add_command(label="Fuente", command=self.formato_fuente)
        formato.add_command(label="Color de texto",
                            command=self.elegir_color_texto)
        formato.add_command(label="Color de fondo",
        ...
        submenu.add_command(label="Restaurar Zoom",
                            command=lambda: self.texto.config(font=(self.f, 12)))

        ver.add_cascade(label="Zoom", menu=submenu)
        ver.add_checkbutton(label="Barra de estado",
                            variable=self.info_estado, command=self.barra_de_estado)

        ayuda = Menu(menu, tearoff=0)
        ayuda.add_command(label="Ver la ayuda")
        ayuda.add_separator()
        ayuda.add_command(label="Acerca del Bloc de notas",
        ...
        self.texto = Text(self.master, font=('Arial', 12), undo=True,
                          insertbackground='red')  # undo = True, selectbackground='yellow'
        self.texto.grid(column=0, row=0, sticky='nsew')
        ladox = Scrollbar(self.master, orient='horizontal',
                          command=self.texto.xview)
        ladox.grid(column=0, row=1, sticky='ew')
        ladoy = Scrollbar(self.master, orient='vertical',
                          command=self.texto.yview)
        ladoy.grid(column=1, row=0, sticky='ns')
        self.texto.configure(xscrollcommand=ladox.set,
    ...
    def salir(self):
        valor = messagebox.askyesno(
            'Salir', '¿Desea Salir?', parent=self.master)
        if valor == True:
            self.master.destroy()
            self.master.quit()
    ...
    def guardar_archivo(self):
        try:
            ...
            messagebox.showinfo(
                'Guardar Archivo', 'Archivo guardado en: ' + str(filename))
        except:
            messagebox.showerror(
                'Guardar Archivo', 'Archivo no guardado\n Error')

    def nueva_ventana(self):
        if self.texto.get != '':
            valor = messagebox.askyesno(
                'Bloc de Notas', '¿Desea guardar el archivo?', parent=self.master)
            if valor == True:
    ...
    def formato_fuente(self):
        ...
        self.combobox_fuente.grid(row=1, column=0, padx=25,
                                  pady=5)  # state="readonly"
        ...
        self.preview = Label(self.vent_tipo_fuente, fg='black',
                             bg='SeaGreen1', font=('Arial', 12))
    ...
    def elegir_color_fondo(self):
        # ff0000 askcolor()[1]   , askcolor()[0] (255.0, 0.0, 0.0)
        color = colorchooser.askcolor()[1]
